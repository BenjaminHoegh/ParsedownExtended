...
class ParsedownExtended extends \ParsedownExtendedParentAlias
{
    ...
    protected function inlineEmojis(array $Excerpt): ?array
    {
        ...
        // Check if the text starts with an emoji code
        foreach ($emojiMap as $code => $emoji) {
            if (substr($Excerpt['text'], 0, strlen($code)) === $code) {
                return [
                    'extent' => strlen($code), // Length of the emoji code
                    'element' => [
                        'text' => $emoji, // Emoji replacement
                    ],
                ];
            }
        }
